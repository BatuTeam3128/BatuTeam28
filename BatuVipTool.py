...
iid =input('~Enter id : ') #luffy is here fuck you 
tok=input('~Enter Token: ') #Luffy is king pro
...
def vip_gmail(email): 
    ...
    if "password"in response:
        ...
        ge+=1
        print(f"\r   < ¦  {F} True  {HH}  {M}[{ge}] ~ {Z} Bad {HH}  {M}[{bt}] ~ {X} Good  {HH}  {M}[{be}] {G} {F} ¦          By : @BatuX28 >\r") 
    else:
        os.system('cls'if os.name=="nt"else"clear")
        be+=1
        print(f"\r   < ¦  {F} True  {HH}  {M}[{ge}] ~ {Z} Bad {HH}  {M}[{bt}] ~ {X} Good  {HH}  {M}[{be}] {G} {F} ¦          By : @BatuX28 >\r") 
